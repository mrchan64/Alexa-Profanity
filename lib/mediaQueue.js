...
var MediaQueue = function(name, config) {
  ...
  this.gapLimit = config.gapLimit || 0;
  ...
  this.queue.log = () => {
    var total = "MediaQueue | "+name.substring(name.length-10)+" | "+queue.length;
    var position = this.position;
    queue.forEach(function(element){
      var pointer = position==element.index() ? ">" : "";
      var queued = element.queued ? "q" : "x";
      total+="\n"+pointer+"\t"+element.index()+" | "+queued+" | "+element.attr+".mp3 | "+element.title+" | "+element.id+" | "+element.timeString+" | "+0;
    })
    console.log(total);
  }
...
MediaQueue.prototype.addFirst = function(res, video){
  ...
  queue[0].queued = true;
  this.death = (new Date()).getTime()+this.queue[this.position].time;
}
...
MediaQueue.prototype.playPrevious = function(res){
  ...
  queue[this.position].queued = true;
  queue[this.position+1].queued = true;
  queue[this.position+2].queued = true;
  res.audioPlayerPlayStream("REPLACE_ALL", stream);
  this.death = (new Date()).getTime()+this.queue[this.position].time;
}
      ...
      resp.on('end', function(){
        ...
        for(var i = 0; i<data.items.length; i++){
          if(data.items[i].id.kind.indexOf('video')!==-1){
            ...
            if(lastInd==-1 || i-lastInd>=mq.gapLimit || i+1==data.items.length){
              console.log('Retrieved result number '+i);
              break;
            }
...
MediaQueue.prototype.initialKey = function(res, keyInfo){
  var mq = this;
  keyInfo.clean = sanitize(keyInfo.title);
  mq.getVideoLength(keyInfo);
  return mq.addFirst(res, keyInfo);
}
